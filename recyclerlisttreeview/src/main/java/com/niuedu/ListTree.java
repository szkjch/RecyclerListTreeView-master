...
public class ListTree {
    public class TreeNode {
        //一个treenode是指代单个节点，应该有数据，view控件id，父节点对象，儿子们的数量，所有子孙的数量，
        //是否显示小箭头，折叠展开用于暂存子孙的list<TreeNode>
        //实际的数据
        private Object data;
        //本node所使用的layout id，即对应的xml布局控件的id
        ...
        //把子孙们收回，输入一个节点集合，将这个集合中的节点遍历存放到collapseDescendant中
        public void retractDescendant(List<TreeNode> collapseChildren) {
            //Must clone a array克隆一个节点的意思
    ...
    public TreeNode addNode(TreeNode parent, Object data, int layoutResId) {
        ...
        //插入非root node，有爹
        if (parent.isExpand()) {
            //yct思路解析：首先找到父亲在list重点位置，然后得到父亲所有子孙的数目，然后插到最后一个子孙的后面
            //如果parent当前状态是展开的
            ...
            //如果parent当前状态是收起的
            //yct思路：用collapseDescendant来暂存treenode节点
